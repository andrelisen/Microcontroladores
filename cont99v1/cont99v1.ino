...
void setup() 
{
  ...
  for(i = 0; i<14;i++)
  {
    digitalWrite(i, OUTPUT);//SETANDO LEDS DO DISPLAY DE 7 SEGMENTOS 
  }  
}
...
void escreveDisplay(byte numero)  //Funcao que aciona o display
{
  ...
  { 
    for (byte letra = 0; letra < 7; ++letra)  
    { 
     digitalWrite(pino2, matrizDireita[numero][letra]);     
      ++pino2;
    }
      digitalWrite(pino, matrizEsquerda[numero][letra]);
      ++pino;
  }
}
